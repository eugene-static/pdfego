...
### Версия v0.1.1
* added:
  - Добавлен параметр `Options.BorderPattern []unit.PT`, позволяющий задать штриховые линии с любым паттерном. Для удобства добавлена функция `BorderPattern(points ...unit.PT) []unit.PT` и переменная `BorderPatternDash`.
  - Добавлен параметр `NodeOptions.BorderColor Color` для задания цвета линий для блоков и слотов.
  - Добавлен параметр `CellOptions.FitContent string`, позволяющий задать перенос текста по словам ("W") или по символам ("S"). Параметр `CellOptions.Wrap bool` упразднен.
  - Добавлен параметр `CellOptions.Underline bool`, позволяющий выводить подчеркнутый текст.
  - Для параметра `CellOptions.Align string` добавлена еще одна опция "J" (justify), позволяющая распределить текст по ширине ячейки.
* fixed:
  - Исправлена работа метода `Constructor.NewPage()`.
